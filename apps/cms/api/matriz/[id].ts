...
import { deleteMatriz, updateMatriz } from '../_lib/matriz.js';
import {
  sendBadRequest,
  sendJson,
...
export default async function handler(
  ...
  if (request.method === 'PATCH') {
    ...
    try {
      const row = await updateMatriz(id, {
      ...
      const message = error instanceof Error ? error.message : 'Erro ao atualizar descritor.';
  ...
  if (request.method === 'DELETE') {
    ...
    if (!serie) {
      return sendBadRequest(response, 'Informe a série para excluir o descritor.');
    }

    try {
      const row = await deleteMatriz(serie, id);
      return sendJson(response, {
        ok: true,
        row,
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Erro ao excluir descritor.';
      return sendBadRequest(response, message);
    }
  }
